<?php
    require 'koneksi.php';
        if (isset($_POST['submit'])) {
			...
			$nama2 = $angka_acak."-kk-".$filename_kk;
			...
			if ($row > 0 ){
				if ($sel['id_jenis_surat'] == $surat){
					...
					<script>
 					...
 					// Simpan ke Database
					$query = mysqli_query($conn, "INSERT INTO pengajuan (tgl_pengajuan, gambar_kk, gambar_akta, nik, id_jenis_surat, catatan) VALUES ('$tgl', '$nama2','$nama1', '$nik','$surat', '$catatan')");
					// Simpan di Folder Gambar
					move_uploaded_file($_FILES['foto_akta']['tmp_name'], "gambar/".$nama1);
					move_uploaded_file($_FILES['foto_kk']['tmp_name'], "gambar/".$nama2);
					echo "
						<script>
